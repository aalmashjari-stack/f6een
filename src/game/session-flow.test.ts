...
describe('ثنائيّات الديربي', () => {
  ...
  it('كلّ أحجام الفرق ٢–٦: لا زوج يتكرّر في الجلسة أبداً', () => {
    /* الإعداد لا يقبل أقلّ من لاعبَين في الفريق (MIN في Setup)، فالأزواج
       أ×ب ≥ ٢×الأكبر ≥ عدد الجولات دائماً — والملاذ في `pickDerbyPair` لا
       يُبلَغ في لعبٍ حقيقيّ. يُفحص هنا بالجولات كلّها لكلّ حجمٍ ممكن. */
    const names = ['A', 'B', 'C', 'D', 'E', 'F']
    for (let a = 2; a <= 6; a++)
      for (let b = 2; b <= 6; b++)
        for (let n = 0; n < 50; n++) {
          const pairs = playDerby([names.slice(0, a), names.slice(0, b)])
          expect(pairs.length).toBe(Math.max(4, a, b))
          expect(new Set(pairs).size, `${a}×${b} الجلسة ${n}: ${pairs.join(' ')}`).toBe(pairs.length)
        }
  })

  it('١×١ (خارج ما يقبله الإعداد): لا مواجهة غيرُ الواحدة — يعود الاختيار على الدورة بلا سقوط', () => {
