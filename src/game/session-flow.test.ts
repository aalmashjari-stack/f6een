...
describe('S1_BACK', () => {
  it('يعيد شاشة كشف السابق ويلغي الاختيار الجاري ونقاط السابق', () => {
    const cats = playableCategories().slice(0, STAGE1_CATEGORIES)
    let s = createSession({ teamNames: ['أ', 'ب'], players: [['1'], ['2']], startingTeam: 0, categories: cats })
    expect(reducer(s, { t: 'S1_BACK' })).toBe(s) // لا سابق

    s = reducer(s, { t: 'S1_PICK', category: cats[0], level: 'سهل' })!
    if (s.phase === 'stage1-letter') s = reducer(s, { t: 'S1_LETTER_DONE' })!
    s = reducer(s, { t: 'S1_TO_REVEAL' })!
    const first = s.currentQuestion!
    const beforeScore = s
    s = reducer(s, { t: 'S1_SCORE', team: 0 })!
    expect(s.teams[0].score).toBeGreaterThan(0)
    expect(s.s1Undo).not.toBeNull()

    s = reducer(s, { t: 'S1_PICK', category: cats[1], level: 'متوسط' })!
    if (s.phase === 'stage1-letter') s = reducer(s, { t: 'S1_LETTER_DONE' })!
    const second = s.currentQuestion!
    expect(s.askedQuestionIds).toContain(second.id)

    const back = reducer(s, { t: 'S1_BACK' })!
    expect(back.phase).toBe('stage1-reveal')
    expect(back.currentQuestion?.id).toBe(first.id)
    expect(back.teams[0].score).toBe(beforeScore.teams[0].score)
    expect(back.s1Index).toBe(beforeScore.s1Index)
    expect(back.s1Played).toEqual(beforeScore.s1Played)
    expect(back.askedQuestionIds).not.toContain(second.id)
    expect(back.usedQuestionIds.has(second.id)).toBe(false)
    /* خطوةٌ واحدة: اللقطة المستعادة بلا لقطة، والرجوع ثانيةً لا يفعل شيئاً. */
    expect(back.s1Undo).toBeNull()
    const again = reducer(back, { t: 'S1_SCORE', team: 1 })!
    expect(again.teams[1].score).toBeGreaterThan(0)
    expect(again.teams[0].score).toBe(beforeScore.teams[0].score)
  })
})
