/**
 * مستويات الأسئلة. **و«تعجيزي» رابعُها** (قرار علي ٩ سبتمبر ٢٠٢٦): صفٌّ
 * خامسٌ في اللوح بخمسين نقطة، «لكي يحسّ المتسابق بأنّه قام بعملٍ كبير».
 *
 * وهو مقبولٌ في النوع والقاعدة قبل أن يُعرض في اللوح — انظر
 * `STAGE1_LEVELS`. فاللعبة لا تعرض فئةً إلّا إذا امتلأت صفوفُها كلُّها،
 * وتشغيلُه قبل امتلاء الخانات يُفرغ اللوح من فئاته كلّها.
 */
export type Level = 'سهل' | 'متوسط' | 'صعب' | 'تعجيزي'
...
export interface Question {
  ...
  image?: string
  /**
   * مفتاح صورة **تظهر مع الإجابة لا مع السؤال** — عكسُ `image`.
   *
   * السؤال نصٌّ خالص («من مؤسّس واتساب؟»)، فإن كُشف ظهر الوجهُ إلى جانب
   * الاسم. ولولا حقلٌ ثانٍ لَما أمكن: `image` وحده يعني «الصورة هي السؤال»،
   * فتعرضها شاشةُ السؤال وتفضح الجواب قبل أن يُسأل.
   *
   * والمفتاح يُحلّ بالسلسلة نفسها (`shippedImage`) — لا مجلّد جديد ولا
   * قاعدةَ تسمية جديدة.
   */
  answerImage?: string
  /**
