-- لسان «الإحصائيات» في اللوحة (طلب علي ٢٣ سبتمبر ٢٠٢٦: «احصائيات فيها جميع
-- انواع الاحصائيات مثل اي فئة تم اختيارها اكثر»).
--
-- **قيمةٌ jsonb واحدة لا صفوف**: كلُّ ما يُرجع صفوفاً يقصّه PostgREST عند ألف
-- بصمت (وقع في `blocked_questions` و`admin_questions`). والحساب كلُّه هنا لا في
-- المتصفّح: الجلسات تحمل حالتها كاملةً (طوابير الأسئلة وسجلّاتها)، ونقلُها
-- كلّها إلى اللوحة لتُعدّ هناك حِملٌ يكبر مع كلّ لعبة.
--
-- **ومصدرُها الجلسات على الخادم وحدها** — أي ألعاب الحسابات المسجّلة. ولا
-- تُحفظ نتيجةُ كلّ سؤالٍ على حدة (المحفوظ مجاميعُ المراحل)، فلا «أصعب سؤال».
--
-- والأيّام والساعات بتوقيت الكويت: `created_at` بالتوقيت العالميّ، ولعبةٌ في
-- الحادية عشرة ليلاً كانت ستُعدّ في اليوم التالي. (وبطاقة «اليوم» القديمة في
-- `admin_stats` تعدّ باليوم العالميّ — تُركت كما هي.)
--
-- `p_exclude_admins`: في يوم الإنشاء 65 من 67 جلسة من حسابات الإدارة — تجاربُ
-- ولعبُ العائلة. فالمفتاح يفصلها حين يكثر اللاعبون الحقيقيّون.
