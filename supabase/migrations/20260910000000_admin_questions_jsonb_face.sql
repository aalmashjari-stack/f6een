/* ═══════════════════════════════════════════════════════════════════════
   ...
   **فمن أعاد تعريفَ دالّةٍ فليأخذ آخرَ ترحيلٍ يذكرها لا أوّلَه** —
   `grep -rl "function public.<اسمها>" supabase/migrations` ثمّ الأحدث
   زمناً.

   **ولا يُكتب رمزُ فتح تعليقٍ ولا رمزُ إغلاقه داخل تعليق**: تعليقاتُ
   Postgres متداخلة، فرمزُ الفتح يفتح تعليقاً ثانياً ورمزُ الإغلاق يغلق
   الخارجَ مبكراً — وكلاهما وقع في هذا الملفّ نفسه. كُتب ليصلح قصَّ
   التصدير عند ألف صفّ، ولم يكن يُطبَّق أصلاً. يحرسه الآن
   `npm run check:sql`، وهو ضمن `npm test`.
